...
- `folders` - top-level organizational unit (unique `name`). `created_at` is immutable; `updated_at` and `last_activity` record the most recent meaningful project activity (video added/add failed/downloaded/download failed/cancelled). `is_admin` splits the table into two workspaces: `false` = public, `true` = the private admin workspace (see "Admin workspace" below).
...
The user-selected local download folder is deliberately **not** a database field or backend setting. It is an optional, browser-owned File System Access handle stored in IndexedDB under its project folder UUID; projects without an override use the browser's default Downloads folder. The browser, not Supabase, controls permission to the user's filesystem.
...
- `components/DownloadQueueContext.tsx` - the core client-side download orchestrator, provided app-wide. Maintains an in-memory job queue (`jobs`) separate from persisted DB statuses (`dbStatuses`), runs **one download at a time** (queue runner effect gates on `activeCount === 0`), polls `/download/progress/{job_id}` every 500ms while a job is downloading, and on completion either writes the file via the File System Access API or falls back to a plain `<a download>` click. Optional directory handles are stored locally in IndexedDB by project folder UUID, so a project without one uses the browser default. A job captures its folder UUID, preventing navigation from redirecting an in-flight download into another project's directory. On loading, immediately after a permission grant, or while writing, an unavailable handle is removed and the UI explains the fallback; deleting a project successfully also removes its local override. Never send a local path or handle to the backend. Every state transition (`pending`/`downloaded`/`cancelled`/`failed`) is synced back to the backend's `video-status` endpoints so progress survives reloads. Terminal download outcomes refresh the active folder header, which renders the activity timestamp in UTC with its concise `last_activity` note.
