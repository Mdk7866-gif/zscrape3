...
- `routes/yt_dlpextractmetadataofvideo.py` — shared yt-dlp metadata extraction (`extract_video_metadata`), not itself a route; per-platform yt-dlp `format`/option tuning (Instagram, Reddit, Twitter/X, YouTube-and-other). Reddit share links (`/s/...`) get resolved via a manual redirect follow before extraction. Reddit gets `backend/cookies.txt` directly for auth; YouTube goes through `extract_with_youtube_fallback()` (see `ytdlp_common.py` below), which only reaches for cookies on a rate-limit-shaped failure.
  ...
  **Instagram duration sometimes comes back as 0** (frontend shows `--:--` for it — see below). yt-dlp's Instagram extractor sources `duration` from a single field (`video_duration`) in Instagram's own API/GraphQL response; when that field is absent — observed on some Reels even though a real DASH manifest is present — yt-dlp has no fallback of its own, even though it already parsed real per-fragment durations out of that manifest while building the format list. `_derive_duration_from_fragments()` in this file recovers it by summing the fragment durations of whichever format has the longest total (the video track, not the shorter audio-only one) — but only when there *are* fragments to sum; some responses have none, and for those the value is genuinely unrecoverable from what yt-dlp returns. Not yet reproduced end-to-end against a real Instagram URL (this dev sandbox is itself blocked from Instagram — see the FortiGuard note above); verified only via direct inspection of the installed yt-dlp source plus a unit test of the summing logic. If it's still wrong after a real test, that's the place to keep digging, not `crudvideos.py` or the frontend.
